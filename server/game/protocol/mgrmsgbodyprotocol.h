#ifndef _MGRMSGBODY_PROTOCOL_H_
#define _MGRMSGBODY_PROTOCOL_H_
#include "msgbase.h"
#include "protocolstruct.h"
/*客户端和服务器通信协议头*/
struct SCSMsgHeadEXReq:public CMsgBase
{
	int8_t m_MsgSize;		/*消息长度*/
	uint8_t m_MsgID;		
	static uint16_t Code(){return 0X0201;}
	SCSMsgHeadEXReq();
	SCSMsgHeadEXReq(
		int8_t MsgSize,
		uint8_t MsgID);
...
/*服务器内部通信协议头*/
struct SSSMsgHeadEXResp:public CMsgBase
{
	...
	static uint16_t Code(){return 0X0202;}
	SSSMsgHeadEXResp();
	SSSMsgHeadEXResp(
		uint64_t roleID,
...
struct STestMsgStructEXNoti:public CMsgBase
{
	uint64_t m_roleID;		/*基础类型*/
	std::string m_name;		/*字符类型*/
	SCSMsgHead m_head;		/*结构体类型*/
	...
	std::map<int32_t,SCSMsgHead> m_maptest1;		/*arr字段取值为vector表示数组 为map表示键值对 arrtype表示后端用的数据类型 如果没有那直接使用arr作为后端数据类型*/
	std::map<int32_t,SCSMsgHead> m_maptest2;		/*arr字段取值为vector表示数组 为map表示键值对 arrtype表示后端用的数据类型 如果没有那直接使用arr作为后端数据类型*/
	static uint16_t Code(){return 0X0203;}
	STestMsgStructEXNoti();
	STestMsgStructEXNoti(
		uint64_t roleID,
		const std::string& name,
		const SCSMsgHead& head,
